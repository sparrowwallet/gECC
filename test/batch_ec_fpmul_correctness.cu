...
#include "batch_fpmul_test_constants.h"
...
// Kernel to perform batch fixed-point scalar multiplication
// All threads multiply the same fixed base point (generator G) by different scalars
// result[i] = scalar[i] * G
__global__ void BatchFixedPointMultiplication(
    const uint32_t *base_point_x,    // X coordinate of fixed base point (single point)
    const uint32_t *base_point_y,    // Y coordinate of fixed base point
    const uint32_t *scalars,         // Scalars to multiply by (column-major if GECC_QAPW_OPT_COLUMN_MAJORED_INPUTS)
    uint32_t *output_points_x,       // X coordinates of results
    uint32_t *output_points_y,       // Y coordinates of results
    uint32_t count) {                // Number of scalar multiplications to perform
  ...
  // Load fixed base point (same for all threads)
  Field px, py;
  // Base point is stored in simple row-major format (single point)
  for (u32 j = 0; j < Field::LIMBS_PER_LANE; ++j) {
    px.digits[j] = base_point_x[j];
    py.digits[j] = base_point_y[j];
  }
  ...
  // Load scalar (different for each thread)
  Field scalar;
...
// Test batch fixed-point scalar multiplication correctness
TEST(BatchECOperations, FixedPointMultiplicationCorrectness) {
  Fq_SECP256K1::initialize();

  const u32 num_tests = BATCH_FPMUL_NUM_TESTS;
  const u32 limbs_per_field = 8; // u32 limbs for 256-bit field

  // Allocate host memory for test data (in u32 format)
  std::vector<uint32_t> h_base_point_x(limbs_per_field);
  std::vector<uint32_t> h_base_point_y(limbs_per_field);
  ...
  // Convert base point from uint64_t to uint32_t format
  convert_u64_to_u32_limbs(BATCH_FPMUL_BASE_POINT_X, h_base_point_x.data(), 4);
  convert_u64_to_u32_limbs(BATCH_FPMUL_BASE_POINT_Y, h_base_point_y.data(), 4);

  // Convert test data from uint64_t to uint32_t format
  for (u32 i = 0; i < num_tests; ++i) {
    convert_u64_to_u32_limbs(&BATCH_FPMUL_SCALARS[i][0], &h_scalars[i * limbs_per_field], 4);
    convert_u64_to_u32_limbs(&BATCH_FPMUL_EXPECTED_X[i][0], &h_expected_x[i * limbs_per_field], 4);
    convert_u64_to_u32_limbs(&BATCH_FPMUL_EXPECTED_Y[i][0], &h_expected_y[i * limbs_per_field], 4);
  }

#ifdef GECC_QAPW_OPT_COLUMN_MAJORED_INPUTS
  // Transform scalars to column-major layout
  ...
  uint32_t *d_base_point_x, *d_base_point_y;
  uint32_t *d_scalars;
  uint32_t *d_results_x, *d_results_y;

  cudaMalloc(&d_base_point_x, limbs_per_field * sizeof(uint32_t));
  cudaMalloc(&d_base_point_y, limbs_per_field * sizeof(uint32_t));
  ...
  // Copy data to device
  cudaMemcpy(d_base_point_x, h_base_point_x.data(), limbs_per_field * sizeof(uint32_t), cudaMemcpyHostToDevice);
  cudaMemcpy(d_base_point_y, h_base_point_y.data(), limbs_per_field * sizeof(uint32_t), cudaMemcpyHostToDevice);
  ...
  BatchFixedPointMultiplication<<<num_blocks, threads_per_block>>>(
    d_base_point_x, d_base_point_y, d_scalars,
  ...
  // Cleanup
  cudaFree(d_base_point_x);
  cudaFree(d_base_point_y);
  cudaFree(d_scalars);
